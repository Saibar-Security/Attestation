...
Pulls saved items from Wallabag and normalises them into Larkspur bookmarks.
...
BASE_URL = "https://app.wallabag.it/api"
TIMEOUT = 15.0
PAGE_SIZE = 100
...
def sync(token: str) -> list[Bookmark]:
    client = WallabagClient(token)
    out = []
    for item in client.iter_items():
        try:
            out.append(client.normalise(item))
        except Exception:  # pragma: no cover - defensive
            log.warning("skipping malformed Wallabag item: %r", item)
    return out
