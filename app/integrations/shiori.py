...
Pulls saved items from Shiori and normalises them into Larkspur bookmarks.
...
BASE_URL = "https://shiori.example.com/api/v1"
TIMEOUT = 15.0
PAGE_SIZE = 100
...
def sync(token: str) -> list[Bookmark]:
    client = ShioriClient(token)
    out = []
    for item in client.iter_items():
        try:
            out.append(client.normalise(item))
        except Exception:  # pragma: no cover - defensive
            log.warning("skipping malformed Shiori item: %r", item)
    return out
