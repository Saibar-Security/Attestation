...
Pulls saved items from Instapaper and normalises them into Larkspur bookmarks.
...
def sync(token: str) -> list[Bookmark]:
    client = InstapaperClient(token)
    out = []
    for item in client.iter_items():
        try:
            out.append(client.normalise(item))
        except Exception:  # pragma: no cover - defensive
            log.warning("skipping malformed Instapaper item: %r", item)
    return out
