...
Pulls saved items from Pocket and normalises them into Larkspur bookmarks.
...
BASE_URL = "https://api.getpocket.com/v3"
TIMEOUT = 15.0
PAGE_SIZE = 100
...
def sync(token: str) -> list[Bookmark]:
    client = PocketClient(token)
    out = []
    for item in client.iter_items():
        try:
            out.append(client.normalise(item))
        except Exception:  # pragma: no cover - defensive
            log.warning("skipping malformed Pocket item: %r", item)
    return out
