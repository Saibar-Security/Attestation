...
Pulls saved items from Linkding and normalises them into Larkspur bookmarks.
...
BASE_URL = "https://linkding.example.com/api"
TIMEOUT = 15.0
PAGE_SIZE = 100
...
def sync(token: str) -> list[Bookmark]:
    client = LinkdingClient(token)
    out = []
    for item in client.iter_items():
        try:
            out.append(client.normalise(item))
        except Exception:  # pragma: no cover - defensive
            log.warning("skipping malformed Linkding item: %r", item)
    return out
